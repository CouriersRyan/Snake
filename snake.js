...
JS_SNAKE.game = (function () {
  ...
  function gameOver() {
    ...
    var centreX = JS_SNAKE.width / 2;
    var centreY = JS_SNAKE.width / 2;
    ctx.fillText('Game Over', centreX, centreY - 30);
        if(score > highscore){
      highscore = score;
      ctx.fillText("New HighScore: " + highscore.toString(), centreX, centreY);
      localStorage.setItem("highscore", score);
    };
    ctx.font = 'bold 15px sans-serif';
    ctx.fillText('Press space to restart', centreX, centreY + 25);
    ctx.restore();
  }
